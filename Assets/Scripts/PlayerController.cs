...
public class PlayerController : MonoBehaviour
{
    ...
    public void OnTriggerEnter2D(Collider2D collision)
    {
         ...
         } else if (collision.CompareTag("LevelEndPortal"))
         {
             LevelEndPortal levelEndPortal = collision.GetComponent<LevelEndPortal>();
             if (levelEndPortal)
            {
                string nextLevel = levelEndPortal.NextLevel.name;
